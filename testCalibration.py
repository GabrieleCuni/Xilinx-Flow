...
def test_calibration(x_test, y_test, y_label):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for img, imagenetId in zip(x_test,y_test):
            ...
            [predId, predLabel] = classIndex[str(predict[0]-1)] # Penso vada messo -1 altrimenti quando predice 1000 va fuori index bound
...
def makeTestSet(imageSize): 
    ...
    print("Start preprocessing the validate calibration images")
    for i in image_names:
        ...
        x_test.append(img)
    print("Stop preprocessing the validate calibration images\n")
...
print(f"\tValidate quantized model: {args.quantize}")
...
# print("x_test[0].shape: ", x_test[0].shape, "y_test[0]: ",y_test[0], "y_label[0]:", y_label[0])
test_calibration(x_test, y_test, y_label)
